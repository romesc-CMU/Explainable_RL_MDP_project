...
path = './draw_map/tiles/tileset/tileset11/ground_water/png/separate/256x256/'
# --------------------------------------------------------------
# A. dirt = road
...
# --------------------------------------------------------------
# B. grass
path = './draw_map/tiles/tileset/tileset11/ground_water/png/separate/256x256/grass_shen_ps/'
...
# --------------------------------------------------------------
# C. stone
path = './draw_map/tiles/tileset/tileset11/ground_stone_house/png/2x/stone/'
...
# D. human and tree
path = './draw_map/tiles/tileset/tileset11/'
...
tree_imgs = [Image.open(path+'tree2_shen_ps.png')]
        ...
        for j in xrange(len(terrains[0])):
            if int(terrains[i][j]) == tree:
                ...
                if i-1>=0 and i-1<len(terrains):
                    # XXX: we don't draw the dirt edge when neighboring with rock, person, tree, or grass
                    if int(terrains[i-1][j]) == grass or \
                            int(terrains[i-1][j]) == person or \
                            int(terrains[i-1][j]) == tree or \
                            int(terrains[i-1][j]) == rock:
                        top = True
                if i+1>=0 and i+1<len(terrains):
                    if int(terrains[i+1][j]) == grass or \
                            int(terrains[i+1][j]) == person or \
                            ...
                            int(terrains[i+1][j]) == rock:
                        bottom = True
                if j-1>=0 and j-1<len(terrains[0]):
                    if int(terrains[i][j-1]) == grass or \
                            int(terrains[i][j-1]) == person or \
                            int(terrains[i][j-1]) == tree or \
                            int(terrains[i][j-1]) == rock:
                        left = True
                if j+1>=0 and j+1<len(terrains[0]):
                    if int(terrains[i][j+1]) == grass or \
                            int(terrains[i][j+1]) == person or \
                ...
                tmp2 = tmp.copy()
                person_img = random.sample(human_imgs,1)[0]
                tmp2.paste(person_img, (0, 0), person_img)
                row_img.append(tmp2)
            elif int(terrains[i][j]) == road:
                top = False
                bottom = False
                left = False
                right = False
                if i-1>=0 and i-1<len(terrains):
                    if int(terrains[i-1][j]) == road:
                        top = True
                if i+1>=0 and i+1<len(terrains):
                    if int(terrains[i+1][j]) == road:
                        bottom = True
                if j-1>=0 and j-1<len(terrains[0]):
                    if int(terrains[i][j-1]) == road:
                        left = True
                if j+1>=0 and j+1<len(terrains[0]):
                    if int(terrains[i][j+1]) == road:
                        right = True
            ...
            elif int(terrains[i][j]) == rock:
                row_img.append(random.sample(stone_img,1)[0])

            elif int(terrains[i][j]) == grass:
                top = False
                bottom = False
                left = False
                right = False

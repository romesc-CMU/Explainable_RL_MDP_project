"""Writes a 256x256 grayscale simplex noise texture file in pgm format
(see http://netpbm.sourceforge.net/doc/pgm.html)
"""
# http://gamedev.stackexchange.com/questions/29044/how-can-i-generate-random-lakes-and-rivers-in-my-game
# $Id: 2dtexture.py 21 2008-05-21 07:52:29Z casey.duncan $
  
import sys
from opensimplex import OpenSimplex

# if len(sys.argv) not in (2, 3) or '--help' in sys.argv or '-h' in sys.argv:
#     print('2dtexture.py FILE [OCTAVES]')
#     print()
#     print(__doc__)
#     raise SystemExit
  
f = open('./opensimplex', 'wt')
gen = OpenSimplex()

f.write('P2\n')
f.write('256 256\n')
f.write('255\n')
for y in range(256):
    for x in range(256):
        f.write("%s\n" % int(gen.noise2d(x, y) * 127.0 + 128.0))
f.close()